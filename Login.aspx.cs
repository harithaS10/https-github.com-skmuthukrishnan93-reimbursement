    ...
    public partial class Login : System.Web.UI.Page
    {
        ...
        protected void btnLogin1_Click(object sender, EventArgs e)
        {
            ...
            if (ValidateUser(username, password, out DataRow userRow))
            {
                ...
                {
                    Response.Redirect("Customercreation.aspx");
                }
                else
                {
        ...
        private bool ValidateUser(string username, string password, out DataRow userRow)
        {
            ...
            {
                string query = "SELECT * FROM Employees WHERE Active = 1 AND OfficialMail = @User AND Password = @Password"; // Use hashed password in production
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@User", username);
                    cmd.Parameters.AddWithValue("@Password", password); // Hash password here in production!
